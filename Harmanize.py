...
def strip_school(name, best_partial_ratio_match):
    ...
    # for each university, we pick only one name for similar second level college/school for consistency.
    # e.g. stern school of business, stern business school will be considered one under "business" type
...
def fuzzy_match(name, loc):
    ...
    sanitize_rules.loc[loc] = decision(name, cleaned_name, best_ratio_match, best_ratio, best_partial_ratio_match,
                                       best_partial_ratio, best_token_sort_ratio_match, best_token_sort_ratio)
    # save the best match ratios for the name in scoreboard data set
    scoreboard.loc[loc] = pd.Series({'name': name, 'best_ratio_match': best_ratio_match, 'best_ratio': best_ratio,
                                     'best_partial_ratio_match': best_partial_ratio_match,
                                     'best_partial_ratio': best_partial_ratio,
                                     'best_token_sort_ratio_match': best_token_sort_ratio_match,
                                     'best_token_sort_ratio': best_token_sort_ratio})
...
def decision(name, cleaned_name, best_ratio_match, best_ratio, best_partial_ratio_match, best_partial_ratio,
             best_token_sort_ratio_match, best_token_sort_ratio, loc):
    ...
    # we'll try to parse the rest of the string as a college or school of the university
        ...
        sanitized_row = pd.Series({
            ...
            'school': strip_school(cleaned_name,
                                   best_partial_ratio_match if best_partial_ratio == 100 else best_token_sort_ratio_match,
                                   loc),
            'confidence': 'high'})
    else:
        ...
        if best_ratio > 90:
            sanitized_row = pd.Series(
                {'orig': name, 'name': best_ratio_match, 'school': '', 'confidence': f'ratio-{best_ratio}'})
        elif best_partial_ratio > 95:
            sanitized_row = pd.Series({'orig': name, 'name': best_partial_ratio_match, 'school': '',
                                       'confidence': f'partial-{best_partial_ratio}'})
        elif best_token_sort_ratio > 95:
            sanitized_row = pd.Series(
                {'orig': name, 'name': best_token_sort_ratio_match, 'school': '',
                 'confidence': f'token-{best_token_sort_ratio}'})
        else:
...
def read_special_matches():
    ...
    special_matches = {k: (v1, v2) for k, v1, v2 in
                       zip(special_match_df.orig, special_match_df.name, special_match_df.school)}
    return special_matches

# read in type of second level college/school, e.g. business, law, etc
...
scoreboard = pd.DataFrame([], columns=['name', 'best_ratio_match', 'best_ratio', 'best_partial_ratio_match',
                                       'best_partial_ratio',
